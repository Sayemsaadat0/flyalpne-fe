import { TCategoryBodyData, THeaderData } from '@/types/types';
import Image from 'next/image';

type TCategoryTableProps = {
  headerData: THeaderData[];
  bodyData: TCategoryBodyData[];
};

const CategoryTable = ({ headerData, bodyData }: TCategoryTableProps) => {
  return (
      ...
      <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        ...
        <tbody>
          {Array.from({ length: 5 }).map((idx) =>
            bodyData.map((bodyItem: TCategoryBodyData) => {
              return (
                <tr
                  key={idx as number}
                ...
                >
                  ...
                  >
                    <Image
                      ...
                      width={100}
                      height={100}
                      alt={bodyItem.categoryName}
                    />
                  </th>
                  <td className="px-6 py-4 whitespace-nowrap">{bodyItem.categoryName}</td>
                </tr>
              );
            }),
          )}
